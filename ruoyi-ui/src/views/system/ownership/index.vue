    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:ownership:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:ownership:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:ownership:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:ownership:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listOwnership, getOwnership, delOwnership, addOwnership, updateOwnership } from "@/api/system/ownership";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOwnership(this.queryParams).then(response => {
        this.ownershipList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOwnership(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOwnership(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/ownership/export', {
        ...this.queryParams
      }, `ownership_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
